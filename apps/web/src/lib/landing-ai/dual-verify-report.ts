...
/** Merge incoming items into report bag; later completed results win over loaded snapshots. */
export function mergeReportItems(
  ...
  for (const item of incoming) {
    ...
    const incomingDone =
      ...
      Boolean(item.landingMessage && item.llmMessage);
    const prevDone =
      prev?.status === 'completed' &&
      Boolean(prev.landingMessage && prev.llmMessage);
...
export function llmBlocksFromReport(
  items: DualVerifyReportItem[],
): ReferenceComplianceBlock[] {
  return blocksFromReport(items, 'llm');
}

export function landingBlocksFromReport(
  items: DualVerifyReportItem[],
): ReferenceComplianceBlock[] {
...
export function buildDualVerifyExecutiveSummary(
  ...
  const lines = [
    ...
    '',
    '**Pass 2 (LLM verify) compliance breakdown** is shown in statistics below. Each point includes full Pass 1 (Landing AI) and Pass 2 (Gemini) cards with status, confidence, fulfilled clauses, and corrective action plans.',
  ];
  ...
  if (review.length > 0) {
    ...
    for (const item of review.slice(0, 15)) {
      lines.push(
        `- **${item.pointId}** — ${item.agreement!.label}: ${item.agreement!.summary}`,
      );
    }
    if (review.length > 15) {
...
/** Parsed compliance rows from Pass 2 (LLM) for workbench-style statistics */
export function parsedResultsFromReport(
  ...
  const blocks =
    pass === 'landing'
      ? landingBlocksFromReport(items)
      : llmBlocksFromReport(items);
