    ...
    ILogger<NdDemoWorkspaceService> logger)
{
    ...
    public async Task<DemoClearResult> ClearDemoWorkspaceAsync(DemoClearRequest req, CancellationToken ct = default)
    {
        ...
        // Each checkbox in the clear form is an independent scope — clearing runs must not
        // also sweep away demo documents just because they happened to be that run's source,
        // and vice versa. Document removal is handled entirely by the clearInternal/clearRegs
        // blocks below, keyed to their own checkboxes.
        if (clearRuns)
        {
            var runs = await ListDemoOwnedAnalysisRunsAsync(demoIds, ct);
            foreach (var run in runs)
            {
                if (await PermanentlyDeleteAnalysisRunAsync(run.Id, ct, purgeUnusedSourceDocs: false))
        ...
        if (clearInternal)
        {
            ...
            var docIds = await ResolveDemoOwnedInternalDocIdsAsync(demoIds, hidden: false, ct);
            var docs = docIds.Count == 0
                ? []
                : await db.StoredDocuments.Where(d => docIds.Contains(d.Id)).ToListAsync(ct);
    ...
    {
        var docIds = await ResolveDemoOwnedInternalDocIdsAsync(demoIds, hidden: true, ct);
        var hidden = docIds.Count == 0
            ? []
            : await db.StoredDocuments.Where(d => docIds.Contains(d.Id)).ToListAsync(ct);
    ...
    /// <summary>
    /// Internal-doc ids owned by demo accounts: uploaded by a demo profile, or a reviewer's
    /// "generated by analysis" corrected copy whose source run belongs to a demo account. The
    /// copy's own UploadedBy is whoever finalized it, which is not always in demoIds, so the
    /// clear/purge queries would otherwise leave these copies behind forever.
    /// </summary>
    private async Task<HashSet<Guid>> ResolveDemoOwnedInternalDocIdsAsync(
        HashSet<Guid> demoIds,
        bool hidden,
        CancellationToken ct)
    {
        var uploadedByDemo = await db.StoredDocuments
            .Where(d =>
                d.UploadedBy != null
                && demoIds.Contains(d.UploadedBy.Value)
                && d.IsHidden == hidden
        ...
        var demoRunIds = (await ListDemoOwnedAnalysisRunsAsync(demoIds, ct))
            .Select(r => r.Id)
            .ToHashSet();
        var generatedCandidates = demoRunIds.Count == 0
            ? []
            : await db.StoredDocuments
                .Where(d =>
                    d.IsHidden == hidden
                    && (d.DocKind == "document" || d.DocKind == "internal")
                    && d.HistoryJson != null
                    && d.HistoryJson.Contains("\"generatedFromRunId\""))
                .Select(d => new { d.Id, d.HistoryJson })
                .ToListAsync(ct);

        var result = new HashSet<Guid>(uploadedByDemo);
        foreach (var candidate in generatedCandidates)
        {
            if (demoRunIds.Any(runId => candidate.HistoryJson!.Contains($"\"generatedFromRunId\":\"{runId}\"")))
    ...
    public async Task<bool> PermanentlyDeleteAnalysisRunAsync(
        Guid runId,
        CancellationToken ct = default,
        bool purgeUnusedSourceDocs = true)
    {
        var run = await db.NdAnalysisRuns.FirstOrDefaultAsync(r => r.Id == runId, ct);
        if (run == null) return false;

        var sourceRegulationDocIds = ParseIdList(run.SelectedRegulationDocIds);
        var sourceInternalDocIds = ParseIdList(run.SelectedInternalDocIds);
        ...
        if (purgeUnusedSourceDocs)
            await PurgeUnusedDemoSourceDocsAsync(sourceRegulationDocIds, sourceInternalDocIds, ct);
        return true;
    }

    private static List<Guid> ParseIdList(string? json)
    {
    ...
    /// <summary>
    /// Deletes the regulation/internal documents that were selected as the run's source, but
    /// only when: the document was uploaded by a demo profile (never a real user's file), no
    /// other analysis run still references it, and it is not one of the shared seed templates
    /// other demo uploads clone parse/points from.
    /// </summary>
    private async Task PurgeUnusedDemoSourceDocsAsync(
        List<Guid> regulationDocIds,
        List<Guid> internalDocIds,
        CancellationToken ct)
    {
        if (regulationDocIds.Count == 0 && internalDocIds.Count == 0) return;

        var demoIds = await demoDirectory.GetDemoProfileIdsAsync(ct);
        var opts = demoOptions.Value;
        var protectedDocIds = new HashSet<Guid>
        {
            opts.DemoInternalTemplateDocumentId,
            opts.DemoRegulationTemplateDocumentId,
            opts.DemoTfsRegulationTemplateDocumentId,
        };

        // A doc id can appear in another still-existing run's selection (shared library
        // upload) — load every remaining run's selections once and check membership.
        var otherRuns = await db.NdAnalysisRuns.AsNoTracking()
            .Select(r => new { r.SelectedRegulationDocIds, r.SelectedInternalDocIds })
            .ToListAsync(ct);
        var regInUseElsewhere = otherRuns.SelectMany(r => ParseIdList(r.SelectedRegulationDocIds)).ToHashSet();
        var internalInUseElsewhere = otherRuns.SelectMany(r => ParseIdList(r.SelectedInternalDocIds)).ToHashSet();

        foreach (var regDocId in regulationDocIds.Distinct())
        {
            if (regInUseElsewhere.Contains(regDocId) || protectedDocIds.Contains(regDocId)) continue;

            var nd = await db.NdRegulationDocuments.AsNoTracking().FirstOrDefaultAsync(d => d.Id == regDocId, ct);
            if (nd == null) continue;

            var uploaderId = nd.CreatedBy;
            if (uploaderId is not Guid uploader && nd.StoredDocumentId is Guid sid)
            {
                uploaderId = await db.StoredDocuments.AsNoTracking()
                    .Where(d => d.Id == sid)
                    .Select(d => d.UploadedBy)
                    .FirstOrDefaultAsync(ct);
            }
            if (uploaderId is not Guid demoUploader || !demoIds.Contains(demoUploader)) continue;
            if (nd.StoredDocumentId is Guid storedId && protectedDocIds.Contains(storedId)) continue;

            try
            {
                await PermanentlyDeleteRegulationDocumentAsync(regDocId, ct);
            }
            catch (Exception ex)
            {
                logger.LogWarning(ex, "Could not purge demo regulation source doc {DocId} after run delete", regDocId);
            }
        }

        foreach (var internalDocId in internalDocIds.Distinct())
        {
            if (internalInUseElsewhere.Contains(internalDocId) || protectedDocIds.Contains(internalDocId)) continue;

            var doc = await db.StoredDocuments.FirstOrDefaultAsync(d => d.Id == internalDocId, ct);
            if (doc == null) continue;
            if (doc.UploadedBy is not Guid demoUploader || !demoIds.Contains(demoUploader)) continue;
            if (!string.IsNullOrWhiteSpace(doc.FileHash) && doc.FileHash == opts.DemoImptfsInternalFileHash) continue;

            try
            {
                var sections = await db.NdInternalDocumentSections
                    .Where(s => s.StoredDocumentId == internalDocId)
                    .ToListAsync(ct);
                if (sections.Count > 0) db.NdInternalDocumentSections.RemoveRange(sections);
            ...
            {
                logger.LogWarning(ex, "Could not purge demo internal source doc {DocId} after run delete", internalDocId);
            }
        }
    }
