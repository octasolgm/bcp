...
export class NdInternalDocumentsComponent implements OnInit, OnDestroy {
  ...
  parseStatusText(doc: InternalDocument): string {
    return doc.parseProgressLabel?.trim() || 'Parsing…';
  }

  parseProgressPct(doc: InternalDocument): number | null {
    if (!this.isParsingDoc(doc)) return null;
    const pct = doc.parseProgressPct;
    if (pct == null || Number.isNaN(pct)) return null;
    return Math.max(0, Math.min(100, pct));
  }
