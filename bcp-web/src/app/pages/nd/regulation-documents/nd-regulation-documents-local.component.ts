...
/**
 * Same document library UI as nd-regulation-documents.component (columns, filters, department,
 * upload, points panel, manual points, analysis history) but "Parse" runs the local pipeline
 * (PdfPig + Tesseract, no Landing AI, no per-page cost, nothing leaves this server) instead of
 * calling Landing AI. Local extraction is a single synchronous call that parses AND extracts points
 * together, so Parse and Extract both trigger the same combined action. Locally-extracted points are
 * NOT written to the regulation_points table Landing AI uses, so they show in this page's points
 * panel but are not yet usable as input to a real analysis run — see
 * docs/discussion/REGUL-PIPELINE-BUILD-PLAN.md.
 */
@Component({
  selector: 'app-nd-regulation-documents-local',
  standalone: true,
...
export class NdRegulationDocumentsLocalComponent implements OnInit, OnDestroy {
  ...
  /** Full local extraction result per document (points come from here, not from Landing AI). */
  ...
  async ngOnInit(): Promise<void> {
    ...
    // route.data emits the current snapshot immediately on subscribe, so calling
    // syncDeletedFromRoute() here too would fire every initial load twice (including the
    // local-status batch call) — subscribe alone covers both the initial and later emissions.
  ...
  startPointsPanelResize(event: MouseEvent): void {
    ...
    const onUp = () => {
      ...
      localStorage.setItem(
        NdRegulationDocumentsLocalComponent.PANEL_SPLIT_KEY,
  ...
  private restorePanelSplit(): void {
    try {
      const saved = localStorage.getItem(NdRegulationDocumentsLocalComponent.PANEL_SPLIT_KEY);
      if (!saved) return;
  ...
  private async mergeLocalStatuses(): Promise<void> {
    ...
    try {
      ...
      for (const doc of this.docs) {
        ...
        doc.extractionStatus = extractStatus === 'extracted' || extractStatus === 'failed' || extractStatus === 'processing'
          ? extractStatus
          : (local.status ?? 'pending').toLowerCase();
        doc.pointCount = local.sectionCount ?? 0;
  ...
  private async runLocalParse(doc: RegulationDocument, event?: Event): Promise<void> {
    ...
    this.patchRegulationDoc(doc, {
      ...
      extractionProgressLabel: 'Parsing locally…',
      extractionProgressPct: null,
    });
    try {
      const res = await this.api.localParseById(storedDocId, this.engine);
      if (!res.success || !res.data) {
        this.error = res.message || `Local parse failed for "${doc.name}".`;
      ...
      console.log(`[local-parse] ${doc.name}`, data);
      const failed = (data.status ?? '').toLowerCase() === 'failed';
      this.patchRegulationDoc(doc, {
        extractionStatus: failed ? 'failed' : 'parsed',
        extractionProgressLabel: failed ? (data.error ?? 'Parse failed') : null,
        extractionProgressPct: null,
        pointCount: 0,
        pageCount: data.totalPages ?? doc.pageCount,
      });
      this.message = failed
        ? `Local parse failed for "${doc.name}".`
        : `Parsed "${doc.name}" (${data.totalPages ?? 0} page(s), local).`;
      if (!failed) this.toast.show(this.message, 'success', 4000);
      await this.loadDocs(true);
    } catch (err) {
      this.error = err instanceof Error ? err.message : `Local parse failed for "${doc.name}".`;
      this.toast.show(this.error, 'error', 6000);
      this.patchRegulationDoc(doc, { extractionStatus: 'failed' });
    } finally {
      this.parsingId = null;
    }
  }

  /** Step 2 — split the already-parsed markdown into clauses/points. Requires Parse to have run first. */
  private async runLocalExtract(doc: RegulationDocument, event?: Event): Promise<void> {
    ...
    this.patchRegulationDoc(doc, {
      extractionStatus: 'processing',
      extractionProgressLabel: 'Extracting points locally…',
      extractionProgressPct: null,
    });
    try {
      const res = await this.api.localExtractById(storedDocId, this.engine);
      if (!res.success || !res.data) {
        this.error = res.message || `Local extraction failed for "${doc.name}".`;
        this.toast.show(this.error, 'error', 6000);
        this.patchRegulationDoc(doc, {
          extractionStatus: 'failed',
          extractionProgressLabel: this.error,
      ...
      const data = res.data;
      this.localResults.set(doc.id, data);
      const failed = (data.extractStatus ?? '').toLowerCase() === 'failed';
      this.patchRegulationDoc(doc, {
        extractionStatus: failed ? 'failed' : 'extracted',
        extractionProgressLabel: failed ? (data.extractError ?? 'Extract failed') : null,
        extractionProgressPct: null,
        pointCount: data.sectionCount ?? 0,
        pageCount: data.totalPages ?? doc.pageCount,
      });
      if (this.selectedDoc?.id === doc.id) {
        this.selectedPoints = this.mapLocalPoints(data);
        this.pointsSource = 'local';
      }
      this.message = failed
        ? `Local extraction failed for "${doc.name}".`
        : `Extracted ${data.sectionCount ?? 0} point(s) from "${doc.name}" (local).`;
      if (!failed) this.toast.show(this.message, 'success', 4000);
      await this.loadDocs(true);
    } catch (err) {
      this.error = err instanceof Error ? err.message : `Local extraction failed for "${doc.name}".`;
      this.toast.show(this.error, 'error', 6000);
      this.patchRegulationDoc(doc, { extractionStatus: 'failed' });
    } finally {
  ...
  private upsertUploadedRegulationDoc(
    ...
    const optimistic: RegulationDocument = {
      id: data.id,
      storedDocumentId: data.storedDocumentId ?? null,
      source: 'nd',
  ...
  async handleExtract(doc: RegulationDocument, event?: Event): Promise<void> {
    await this.runLocalExtract(doc, event);
  }
  ...
  /** Full parsed markdown (with page refs) for the selected doc, once Parse has run — independent of Extract. */
  ...
  async handleParse(doc: RegulationDocument, event?: Event): Promise<void> {
    await this.runLocalParse(doc, event);
  }
