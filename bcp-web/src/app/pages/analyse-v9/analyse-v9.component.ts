...
export class AnalyseV9Component extends AnalyseBase implements OnInit, OnDestroy {
  readonly versionLabel = 'V9 - Analysis Version 2';
  readonly versionPath = '/analyse-v9';
  ...
  private canonicalizeNdAnalysisUrl(): void {
    ...
    void this.router.navigate(['/nd/analyse-v9'], {
      replaceUrl: true,
  ...
  override pointSnapshotForPointId(pointId: string): PointSnapshot | null {
  ...
  private indexNdRunPoints(points: AnalysisPoint[]): void {
    ...
    for (const p of points) {
      ...
      for (const g of this.govPoints) {
        ...
        const matches =
          (snap.pointNumber && (g.point_id === snap.pointNumber || section === snap.pointNumber)) ||
          (p.regulationPointId &&
            (g.point_id === p.regulationPointId || sourced.regulationPointId === p.regulationPointId)) ||
          (p.id && g.point_id === p.id);
  ...
  protected override onNdRunAttached(runId: string, points: AnalysisPoint[]): void {
    if (points.length) this.indexNdRunPoints(points);
  ...
  private buildSessionAnalysisPoint(pointId: string): AnalysisPoint | null {
    ...
    return {
      ...
      pointSnapshot: JSON.stringify({
        pointNumber: gov?.section?.trim() || pointId,
  ...
  private async runNdShellAnalysis(): Promise<void> {
    ...
    this.ndRunId = runId;
    await this.router.navigate(['/nd/analyse-v9'], {
